...
<div id="webhtml" >
  ...
  <div class="box_con">
    <div class="con_top"><a href="<?php echo ($siteinfo["site_url"]); ?>"><?php echo ($siteinfo["site_name"]); ?></a>&nbsp;&gt;&nbsp;<a href="<?php echo ($novelinfo["classurl"]); ?>"><?php echo ($novelinfo["classname"]); ?></a>&nbsp;&gt;&nbsp;<?php echo ($novelinfo["novelname"]); ?></div>

    <div id="maininfo">
      <div id="fmimg"><img alt="<?php echo ($novelinfo["novelname"]); ?>" src="<?php echo ($novelinfo["novelimg"]); ?>"></div>
      <div id="info" style="height: auto">
        <h1><?php echo ($novelinfo["novelname"]); ?></h1>
        <p><B>作者：<?php echo ($novelinfo["novelauthor"]); ?></B> <label class="zhanwei"></label>更新时间：<?php echo (date('Y-m-d H:i:s',$novelinfo["update_time"])); ?></p>
		<p>小说类别：<a href="<?php echo ($novelinfo["classurl"]); ?>"><?php echo ($novelinfo["classname"]); ?></a><label class="zhanwei"></label> 状态：<?php if($novelinfo["novelstate"] == 0): ?>连载中<?php else: ?>已完本<?php endif; ?> </p>
        <p>今日阅读：<?php echo ($novelinfo["clicktoday"]); ?> <label class="zhanwei"></label>本月阅读：<?php echo ($novelinfo["clickmonth"]); ?> <label class="zhanwei"></label>总阅读数：<?php echo ($novelinfo["clicksum"]); ?></p>
  ...
  <div class="box_con">
    <div id="list">
		<?php if(is_array($chapters)): foreach($chapters as $key=>$chapter): ?><h2><?php echo ($chapter["volname"]); ?></h2>
			<dl>
			<?php if(is_array($chapter[0])): foreach($chapter[0] as $key=>$con): ?><dd><a href="<?php echo ($con["con_url"]); ?>"  title="<?php echo ($con["con_name"]); ?>"><?php echo ($con["con_name"]); ?></a></dd><?php endforeach; endif; ?>
...
<div id="msg">  
	...
	<script>
	  (function(){
		var appid = 'cyrmeQ6jB',
		conf = 'prod_dd47d91caada5d3dda12619a6fab549d';
		var doc = document,
